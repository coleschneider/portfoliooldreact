...
const OBSERVER_MAP: Map<string, ResizeObserver> = new Map()
...
export function observe(element: Element, callback: ObserverInstanceCallback, options: IntersectionObserverInit = {}) {
  ...
  if (!observerInstance) {
    // eslint-disable-next-line
        observerInstance = new ResizeObserver(onChange, options);
  ...
  const instance: ObserverInstance = {
    callback,
    element,
    dimensions: { top, right, bottom, left, width, height },
    observerId,
...
function onChange(changes: ResizeObserverEntry[]) {
  changes.forEach(resize => {
    const { target } = resize
    const instance = INSTANCE_MAP.get(target)
    if (instance) {
      const { top, right, bottom, left, width, height } = target.getBoundingClientRect()
      instance.dimensions = { top, right, bottom, left, width, height }
      instance.callback({ top, right, bottom, left, width, height }, resize)
    }
  })
}
